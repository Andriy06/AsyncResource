// An async wrapper over a thread-unsafe resource that needs to be accessed concurrently.
...
// Using `Sync()` is the same as using a reader-writer mutex to synchronize access to the resource, 
// but it's impossible to forget to acquire mutex.
...
class TAsyncResource
{
public:
    template<typename... ArgTypes>
    TAsyncResource(ArgTypes&&... ResourceCtorArgs);
...
int main()
{
    TAsyncResource<FResource> Resource{ 0 };

    // the following code is thread-safe

    // read-only (RO) tasks take `const&` to the resource, and are executed concurrently
    TTask<int> Read1 = Resource([](const FResource& Resource) { return Resource.Read(); });
    TTask<int> Read2 = Resource([](const FResource& Resource) { return Resource.Read(); });
    
    // read-write (RW) tasks take non-const reference and are executed exclusively
    TTask<void> Write1 = Resource([](FResource& Resource) { Resource.Write(42); });
